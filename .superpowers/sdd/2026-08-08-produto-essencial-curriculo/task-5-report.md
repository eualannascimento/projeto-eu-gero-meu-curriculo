...
3. `tests/smoke-test.js` cobre `javascript:`, `data:`, `vbscript:`, URL sem
   esquema e URL HTTPS, inclusive com PDFs reais.
...
2. `node tests/smoke-test.js`: 311 testes aprovados, 0 falhas.
3. `node tests/pdf-layout-validation.js`: os cinco layouts continuam A4, com
   uma página e texto extraível.
4. Inspeção das anotações reais no smoke test: PDFs com `javascript:`,
   `data:` e `vbscript:` não contêm `/URI`; URL sem esquema gera `/URI` com
   destino HTTPS normalizado; URL HTTPS válida mantém anotação.
